...
const API_WEATHER_URL = `http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=`
...
export default function App () {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if(!city.trim()) throw { message: 'Obligatory field!' }
      const response = await fetch(`${API_WEATHER_URL}${city}`)
      const data = await response.json()
      if (data.error) throw { message: data.error.mesage }
      setWeather({
